...
- **macOS focus / popover dismissal**: show the pet with AppKit `orderFrontRegardless` (`tray.rs::show_pet_without_activating`), NEVER Tauri `show()` — `show()` maps to `makeKeyAndOrderFront:`, steals key-window status, and so blurs+dismisses the open sessions popover. The popover is dismissed by a global `NSEvent` mouse monitor (`tray.rs::install_popover_dismiss_monitor`, fires on a click in any OTHER app), not a webview blur listener. Don't reintroduce blur-based hide or `win.show()` for the pet.
